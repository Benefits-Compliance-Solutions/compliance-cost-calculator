 ...
 * to encourage users to complete all 5 input sections for a full analysis.
 * Each section contributes 20% to the total confidence score.
 * Uses BCS brand colors and corporate minimalist design.
 */

import { CheckCircle2, Circle, ChevronRight } from "lucide-react";

export interface SectionsReviewed {
  basics: boolean;
...
const SECTION_LABELS: { key: keyof SectionsReviewed; label: string }[] = [
  ...
  { key: "staffTime",         label: "Staff Time" },
...
function getAccuracyLabel(pct: number): {
  ...
  if (pct >= 80)   return { label: "Nearly Complete",    color: "text-blue-700",     bg: "bg-blue-50 border-blue-200",        bar: "bg-blue-500" };
  if (pct >= 60)   return { label: "Developing",         color: "text-sky-700",      bg: "bg-sky-50 border-sky-200",          bar: "bg-sky-500" };
  if (pct >= 40)   return { label: "Partial Estimate",   color: "text-amber-700",    bg: "bg-amber-50 border-amber-200",      bar: "bg-amber-500" };
  if (pct >= 20)   return { label: "Early Estimate",     color: "text-orange-700",   bg: "bg-orange-50 border-orange-200",    bar: "bg-orange-400" };
...
export default function AnalysisAccuracy({
  sectionsReviewed,
  completionPct,
}: AnalysisAccuracyProps) {
  const accuracy = getAccuracyLabel(completionPct);
  const isComplete = completionPct === 100;

  return (
    <div
      className={`rounded-xl border p-4 mb-4 transition-all duration-500 ${accuracy.bg}`}
    >
      {/* Header row */}
      ...
      {/* Section checklist — 5 items in a 2-col + 1 layout */}
      ...
      {!isComplete && (
        <div className="flex items-start gap-1.5 pt-2 border-t border-black/10">
          ...
          <p className="text-xs text-muted-foreground leading-snug">
            Complete all 5 sections on the left for a fully personalized
            estimate. Uncompleted sections use industry default values.
          </p>
        </div>
      )}
      ...
      {isComplete && (
        <div className="flex items-center gap-1.5 pt-2 border-t border-emerald-200">
          ...
          <p className="text-xs text-emerald-700 font-medium">
            All 5 sections reviewed — this is your personalized analysis.
          </p>
        </div>
      )}
    </div>
